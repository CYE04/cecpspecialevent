...
`;
  ...
  wrapper.addEventListener('click', function (e) {
    ...
    /* Modal — 直接读取元素内的文字，不依赖 data-modal-body 属性 */
    ...
    if (trigger) {
      ...
      // 优先读 data-modal-body，若无则提取元素内所有文字段落
      var bodyAttr = trigger.getAttribute('data-modal-body');
      var bodyHtml;
      if (bodyAttr) {
        bodyHtml = bodyAttr;
      } else {
        // 把元素内所有文字节点包成 <p>
        bodyHtml = '<p>' + (trigger.textContent || '').trim().replace(/\n+/g, '</p><p>') + '</p>';
      }
